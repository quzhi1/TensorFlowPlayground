    ...
    if job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(
            ...
            with tf.name_scope('layer'):
                # Variables of the hidden layer 1
                with tf.name_scope('hidden_layer 1'):
                    hid_w_1 = tf.Variable(
                        tf.truncated_normal([IMAGE_PIXELS * IMAGE_PIXELS, hidden_units], stddev=1.0 / IMAGE_PIXELS),
                        name="hid_w_1")
                    hid_b_1 = tf.Variable(tf.zeros([hidden_units]), name="hid_b_1")
                    tf.summary.histogram("hidden_weights_1", hid_w_1)
                    hid_lin_1 = tf.nn.xw_plus_b(x, hid_w_1, hid_b_1)
                    hid_1 = tf.nn.relu(hid_lin_1)

                # Variables of the hidden layer 2
                with tf.name_scope('hidden_layer 2'):
                    hid_w_2 = tf.Variable(
                        tf.truncated_normal([IMAGE_PIXELS * IMAGE_PIXELS, hidden_units], stddev=1.0 / IMAGE_PIXELS),
                        name="hid_w_2")
                    hid_b_2 = tf.Variable(tf.zeros([hidden_units]), name="hid_b_2")
                    tf.summary.histogram("hidden_weights", hid_w_2)
                    hid_lin_2 = tf.nn.xw_plus_b(hid_1, hid_w_2, hid_b_2)
                    hid_2 = tf.nn.relu(hid_lin_2)

                # Variables of the softmax layer
                with tf.name_scope('softmax_layer'):
                    sm_w = tf.Variable(tf.truncated_normal([hidden_units, 10], stddev=1.0 / math.sqrt(hidden_units)),
                                       name="sm_w")
                    sm_b = tf.Variable(tf.zeros([10]), name="sm_b")
                    tf.summary.histogram("softmax_weights", sm_w)
                    y = tf.nn.softmax(tf.nn.xw_plus_b(hid_2, sm_w, sm_b))
